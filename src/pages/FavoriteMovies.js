      ...
      <div className='row m-5 text-center overflow-scroll w-100 overflow-x-hidden Slide border-top border-bottom' >
        {   
            ...
            favorites.map((favorite, i) =>{
              {
                console.log(favorite) //favorite kthen objektin e filmave qe i kemi bo favorite 
              }
              <div key={i} className='col-2 mb-1 p-5'>
                <FavoritesCard {...favorite}/> 
              </div>
            })
            : <div><h1 className='text-secondary text-align-center'>Favorite list is empty...</h1></div>
        }
      </div>
    </div>
  )
